...
export default function AILikelihoodCard({ aiScore, className = '', showDetectors = true }: AILikelihoodCardProps) {
  ...
  const dominantColor =
    breakdown.dominantType === 'human' ? 'text-emerald-600 dark:text-emerald-400' :
    breakdown.dominantType === 'ai' ? 'text-rose-600 dark:text-rose-400' :
    'text-amber-600 dark:text-amber-400';

  const dominantBgColor =
    breakdown.dominantType === 'human' ? 'bg-emerald-500' :
    breakdown.dominantType === 'ai' ? 'bg-rose-500' :
    'bg-amber-500';

  const dominantLabel =
    breakdown.dominantType === 'human' ? 'Human' :
    breakdown.dominantType === 'ai' ? 'AI' :
    'Mixed';

  const dominantPercentage =
    breakdown.dominantType === 'human' ? breakdown.humanPercentage :
    breakdown.dominantType === 'ai' ? breakdown.aiPercentage :
        ...
        <div className="relative">
          ...
          <svg className="h-40 w-40 -rotate-90 transform">
            <circle
              ...
              strokeWidth="12"
              fill="none"
              className="text-slate-200 dark:text-white/[0.1]"
            />
            {/* Progress circle */}
            <circle
              cx="80"
              ...
              strokeWidth="12"
              fill="none"
              strokeDasharray={`${(dominantPercentage / 100) * 440} 440`}
              className={dominantBgColor}
              strokeLinecap="round"
            />
          </svg>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <div className={`text-4xl font-bold ${dominantColor}`}>
              {dominantPercentage}%
            </div>
            <div className={`text-sm font-semibold ${dominantColor}`}>
              {dominantLabel}
            </div>
          </div>
        </div>
